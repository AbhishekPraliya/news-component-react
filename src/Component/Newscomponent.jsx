/* eslint-disable array-callback-return */
import React,{Component} from 'react'
import Newsitem from "./NewsItemBox"
import Spinner from './Spinner';
class About extends Component {

  constructor(){
    super();
    // console.log("This is constructor")
    this.state={
      articles:[],
      loading:false,
      page:1
    }
  }
  async componentDidMount(){
    console.log(`${process.env.NEWS_API_KEY}`)
    let url=`https://newsapi.org/v2/everything?q=apple&from=2024-07-19&to=2024-07-19&sortBy=popularity&apiKey=${process.env.NEWS_API_KEY}&pageSize=15&page=${this.state.page}`
    this.setState({loading:true});
    let data=await fetch(url);
    let newsData=await data.json();
    // console.log(newsData);
    this.setState({
      articles:newsData.articles,
      loading:false
    })
  }

  noImageUrl=("data:image/jpeg;base64,")
  
  handelPreviClick = async ()=>{
    let url=`https://newsapi.org/v2/everything?q=apple&from=2024-07-19&to=2024-07-19&sortBy=popularity&apiKey=875811e0d5bc472a956df6e917a02da4&pageSize=15&page=${this.state.page - 1}`
    this.setState({loading:true});
    let data=await fetch(url);
    let newsData=await data.json();

    this.setState({
      loading:false,
      page:this.state.page-1,
      articles:newsData.articles
    })
    // console.log("Previ");
  }

  handelNextClick = async ()=>{
    let url=`https://newsapi.org/v2/everything?q=apple&from=2024-07-19&to=2024-07-19&sortBy=popularity&apiKey=875811e0d5bc472a956df6e917a02da4&pageSize=15&page=${this.state.page + 1}`
    this.setState({loading:true});
    let data=await fetch(url);
    let newsData=await data.json();
    this.setState({
      loading:false,
      page:this.state.page+1,
      articles:newsData.articles
    })
    // console.log("Next");
  }
  render(){
    // console.log(`page=${this.state.page}`);
    return (
      <div className='container my-3'>
        <h2 className='text-center'>NewsWeb-Top Heading</h2>
        <div className="container" style={{height:"40px"}}>
          <button disabled={this.state.page<=1} type="button" onClick={this.handelPreviClick} class="btn btn-dark" style={{position:"absolute",left:"120px"}}>&larr; Previ</button>
          {this.state.loading && <Spinner/>}
          <button disabled={this.state.page>=6} type="button" onClick={this.handelNextClick} class="btn btn-dark" style={{position:"absolute",right:"120px"}}>Next &rarr;</button>
        </div>
        <div className="row">
          {this.state.articles && this.state.articles.map((element,key)=>{
            return( 
              <div className="col-md-4" key={key}>
                <Newsitem
                  title={element.title?element.title.slice(0,40):"news"}
                  description={element.description?element.description.slice(0,95):"news"}
                  imageUrl={element.urlToImage?element.urlToImage:this.noImageUrl}
                  newsUrl={element.url}
                  num={key}
                />
              </div>
            )
          })}
        </div>
        <div className="container" style={{height:"40px"}}>
          <button disabled={this.state.page<=1} type="button" onClick={this.handelPreviClick} class="btn btn-dark" style={{position:"absolute",left:"120px"}}>&larr; Previ</button>
          <button disabled={this.state.page>=6} type="button" onClick={this.handelNextClick} class="btn btn-dark" style={{position:"absolute",right:"120px"}}>Next &rarr;</button>
        </div>
      </div>
    )
  }
}


export default About;
